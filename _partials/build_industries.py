...
INDUSTRIES = [
    {
        ...
        # Optional: filename in /assets/images/ to run behind this page's hero.
        # Industries without one keep the flat midnight hero.
        "hero_image": "industry-technology.jpg",
...
def hero_photo_style(ind):
    """Per-industry hero photograph, when one has been supplied for that page.

    Emits a phone variant alongside the desktop file: `foo.jpg` -> `foo-m.jpg`,
    rendered at the exact device-pixel box so the browser blits it 1:1 instead
    of upscaling a desktop-width image on a 3x display.
    """
    img = ind.get("hero_image")
    if not img:
        return ""
    mobile = img.rsplit(".", 1)[0] + "-m." + img.rsplit(".", 1)[1]
    return f"""
.page-hero{{
    background-image:
    ...
    background-position:{ind.get("hero_focus", "center")};
}}
...
"""
    ...
    html = build(
        ...
        extra_style=PAGE_STYLE + hero_photo_style(ind),
        body_html=body,
        section="industries",
