...
def article_rows(articles):
    ...
    for i, a in enumerate(articles, start=1):
        rows.append(f"""
<a href="/resources/{a['slug']}/" class="editorial-row" style="text-decoration:none; color:inherit;">
<div class="row-num">{i:02d}</div>
<div>
<span class="cat-tag">{a['category_label']}</span>
<h3>{a['title']}</h3>
<p>{a['description']}</p>
<span class="row-link">Read Article &rarr;</span>
</div>
</a>""")
...
def build_hub():
    cat_rows = []
    for i, c in enumerate(CATEGORIES.values(), start=1):
        cat_rows.append(f"""
<a href="/{c['path']}/" class="editorial-row" style="text-decoration:none; color:inherit;">
<div class="row-num">{i:02d}</div>
<div>
<h3>{c['label']}</h3>
<p>{c['intro']}</p>
<span class="row-link">Browse {c['label']} &rarr;</span>
</div>
</a>""")
    cat_cards = f'<div class="editorial-list">{"".join(cat_rows)}</div>'

    featured = article_rows(ARTICLES)
...
def build_category(key, cat):
    ...
    cards = article_rows(cat_articles)

    extra = ""
    if key == "industry-insights":
        names = [("Technology","technology"),("Healthcare","healthcare"),("Sales","sales"),("Operations","operations"),("Administrative","administrative")]
        links = ", ".join(f'<a href="/industries/{slug}/">{name}</a>' for name, slug in names[:-1])
        last_name, last_slug = names[-1]
        extra = f"""
<section class="tint">
<div class="container">
<div class="section-label">BROWSE BY INDUSTRY</div>
<h2 class="section-title">See staffing pages for each focus area.</h2>
<p class="industries-line">{links}, and <a href="/industries/{last_slug}/">{last_name}</a>.</p>
</div>
</section>
"""
