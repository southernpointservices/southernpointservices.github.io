...
def service_split(label, title, body_html, dark=False):
...
<h2 class="section-title" style="max-width:760px;{' color:#fff;' if dark else ''}">{title}</h2>
{body_html}
</div>
</section>"""
...
MATCH_STEPS = [
    ("01", "Identify", "We start by understanding the actual requirement — the role, the scope, the timeline — not a generic template applied regardless of fit."),
...
def match_steps():
    out = []
    for num, title, desc in MATCH_STEPS:
        out.append(f"""
<div class="step">
...
<div class="stat"><strong>Four Service Areas</strong><span>Staffing, professional, workforce &amp; specialized</span></div>
<div class="stat"><strong>Built to Fit</strong><span>Around the requirement, not a template</span></div>
...
<h2 class="section-title" style="max-width:760px;">Built to match the work — not a fixed process forced onto it.</h2>
